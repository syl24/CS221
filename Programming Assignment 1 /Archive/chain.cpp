...
void Chain::moveBack(int startPos, int len, int dist){
    ...
    if (empty() || len == 0 || dist == 0 || dist >= size()){
        return;
    }
...
void Chain::roll(int k){
    ...
    int kint = size() - k;
    Node * kk = walk(head_, kint+1);
    Node * b4 = walk(head_, kint);
    Node * end = walk(head_, size());
...
void Chain::weave(Chain & other) { // leaves other empty.
    ...
    //             this->tail_->prev->next = othernode;
    //             othernode->prev = this->tail_->prev;
    //             this->tail_->prev = other.tail_->prev;
    //             other.tail_->prev->next = this->tail_;
